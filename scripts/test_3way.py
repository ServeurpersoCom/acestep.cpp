#!/usr/bin/env python3
"""
test_3way.py: minimal 3-way comparison CUDA / GGML / Python.

Calls C++ binaries directly (no copy).
Uses ACE-Step handler for Python (no inference reimplementation).
Monkey-patches model methods to dump intermediates in binary format.

Usage:
    python3 scripts/test_3way.py                      # all 3
    python3 scripts/test_3way.py --skip-cuda           # GGML + Python only
    python3 scripts/test_3way.py --skip-python         # CUDA + GGML only
    python3 scripts/test_3way.py --skip-ggml           # CUDA + Python only
"""
import os, sys, struct, subprocess, shutil, argparse
import numpy as np
...
CUDA_BIN   = os.path.join(ROOT, "cuda", "dit-vae")
...
DUMP_CUDA   = os.path.join(SCRIPT_DIR, "cuda")
...
def load_dump(path):
    """Load raw f32 data from binary dump (skip header)."""
...
def run_binary(binary, label, dump_dir, caption, lyrics, bpm, duration, seed,
               noise_file=None):
    """Run a C++ binary with --dump. Returns True on success."""
...
def main():
    ...
    ap.add_argument("--skip-ggml",   action="store_true")
    ...
    ap.add_argument("--noise-file",  default=None,
                    help="bf16 noise file for C++ binaries (default: auto for seed 42)")
    args = ap.parse_args()

    # Auto-detect Philox noise file for seed 42
    ...
    if not args.skip_cuda:
        if os.path.isdir(DUMP_CUDA):
            shutil.rmtree(DUMP_CUDA)
        print("\n" + "=" * 60 + "\nCUDA\n" + "=" * 60)
        if run_binary(CUDA_BIN, "CUDA", DUMP_CUDA, args.caption, args.lyrics,
    ...
    if not args.skip_ggml:
        ...
        print("\n" + "=" * 60 + "\nGGML\n" + "=" * 60)
        if run_binary(GGML_BIN, "GGML", DUMP_GGML, args.caption, args.lyrics,
                      args.bpm, args.duration, args.seed,
                      noise_file=noise_file):
            dirs["GGML"] = DUMP_GGML
    ...
    if len(dirs) < 2:
        print(f"\nNeed at least 2 backends, got {len(dirs)}: {list(dirs.keys())}")
        return 1

    print("\n" + "=" * 60)
    print(f"COMPARISON ({' vs '.join(dirs.keys())})")
    print("=" * 60)
    compare(dirs)
    return 0

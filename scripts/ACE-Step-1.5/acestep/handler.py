...
class AceStepHandler(DiffusionMixin, InitServiceMixin, LoraManagerMixin, ProgressMixin):
    ...
    def _parse_audio_code_string(self, code_str: str) -> List[int]:
        """Extract integer audio codes from <|audio_code_123|> or comma-separated ints."""
        if not code_str:
            return []
        try:
            ...
            codes = []
            clamped_count = 0
            token_codes = re.findall(r"<\|audio_code_(\d+)\|>", code_str)
            if token_codes:
                raw_values = [int(x) for x in token_codes]
            else:
                raw_values = [int(x.strip()) for x in code_str.split(",") if x.strip().isdigit()]
            for code_value in raw_values:
                clamped_value = max(0, min(code_value, MAX_AUDIO_CODE))
                if clamped_value != code_value:
                    clamped_count += 1
                codes.append(clamped_value)
            if codes:
                logger.info(f"[_parse_audio_code_string] Parsed {len(codes)} audio codes")
            return codes
        except Exception as e:
            logger.debug(f"[_parse_audio_code_string] Failed to parse: {e}")
            return []
    ...
    def _prepare_batch(
        ...
        for i in range(batch_size):
            if audio_code_hints[i] is not None:
                ...
                if hints is not None:
                    ...
                    logger.info(f"[DEBUG LM_HINTS] hints: shape={hints.shape} dtype={hints.dtype} nan={torch.isnan(hints).sum().item()}")
        ...
        if any(h is not None for h in precomputed_lm_hints_25Hz_list):
            ...
            logger.info(f"[DEBUG LM_HINTS] precomputed_lm_hints_25Hz: shape={precomputed_lm_hints_25Hz.shape} nan={torch.isnan(precomputed_lm_hints_25Hz).sum().item()}")
        else:
    ...
    def generate_music(
        ...
        try:
            ...
            try:
                ...
                # DEBUG HOOKS: trace NaN in encoder sub-modules
                _debug_hooks = []
                def _make_hook(name):
                    def _hook(module, inputs, output):
                        ...
                        def _check(prefix, t):
                            if isinstance(t, torch.Tensor):
                                ...
                                logger.info(f"[HOOK] {prefix}: shape={t.shape} dtype={t.dtype} nan={nan_count} min={t[~torch.isnan(t)].min().item() if nan_count < t.numel() else float('nan'):.4f} max={t[~torch.isnan(t)].max().item() if nan_count < t.numel() else float('nan'):.4f}")
                            elif isinstance(t, (tuple, list)):
                                for i, tt in enumerate(t):
                                    _check(f"{prefix}[{i}]", tt)
                        _check(f"{name}.input", inputs)
                        if isinstance(output, torch.Tensor):
                            _check(f"{name}.output", output)
                        elif isinstance(output, (tuple, list)):
                            for i, o in enumerate(output):
                                _check(f"{name}.output[{i}]", o)
                        elif hasattr(output, 'last_hidden_state'):
                            _check(f"{name}.output.last_hidden_state", output.last_hidden_state)
                    return _hook
                # Hook encoder sub-modules
                enc = self.model.encoder
                _debug_hooks.append(enc.text_projector.register_forward_hook(_make_hook("text_projector")))
                _debug_hooks.append(enc.lyric_encoder.register_forward_hook(_make_hook("lyric_encoder")))
                _debug_hooks.append(enc.timbre_encoder.register_forward_hook(_make_hook("timbre_encoder")))
                # Also hook the full encoder
                _debug_hooks.append(enc.register_forward_hook(_make_hook("encoder_full")))
